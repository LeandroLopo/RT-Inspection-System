...
\begin{titlepage}
    ...
    \begin{tabular}{ll}
        Integrante 1: & Nome completo -- Matrícula 000000 \\
        Integrante 2: & Nome completo -- Matrícula 000000 \\
        Integrante 3: & Nome completo -- Matrícula 000000 \\
    \end{tabular}
...
Na Etapa 1 foi adotada uma arquitetura multitarefa em processo único. As tarefas internas do robô, correspondentes aos blocos de controle embarcado, foram implementadas como threads C++ dentro do processo principal. A comunicação entre essas tarefas ocorre por memória compartilhada, com buffers protegidos por \texttt{std::mutex} e \texttt{std::condition\_variable}.

Essa decisão é adequada para a primeira etapa porque permite demonstrar diretamente os conceitos de concorrência, região crítica, produtor-consumidor e sincronização. A comunicação via IPC/MQTT, bem como as interfaces gráficas de simulação e operação remota, ficam planejadas para a Etapa 2.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \node[state, below=of rec] (robotstate) {SharedRobotState\\posicao, velocidade, estados\\mutex};
    ...
    \caption{Arquitetura detalhada da Etapa 1, com threads, buffers, estados compartilhados e mecanismos de sincronização.}
...
\texttt{ReconstrucaoSuperficie} & Thread C++ & Consome leituras de LIDAR, aplica média móvel, gera pontos de superfície e detecta anomalias. \\
\texttt{DistanciaPercorrida} & Thread C++ & Consome o encoder e calcula a posição horizontal do robô. \\
...
\texttt{ControleNavegacao} & Thread C++ cíclica & Aplica controle proporcional simples e gera aceleração. \\
...
\midrule
\endhead
\texttt{SensorBuffer} & \texttt{SimulacaoSensores -> ReconstrucaoSuperficie} & \texttt{mutex\_sensor} e \texttt{dado\_disponivel\_var}. \\
\texttt{EncoderBuffer} & \texttt{SimulacaoSensores -> DistanciaPercorrida} & \texttt{mutex\_encoder} e \texttt{encoder\_disponivel\_var}. \\
\texttt{SurfaceBuffer} & \texttt{ReconstrucaoSuperficie -> ColetorDados} & \texttt{mutex\_superficie} e \texttt{surface\_point\_var}. \\
\texttt{CameraEvent} & \texttt{ReconstrucaoSuperficie -> InspecaoCamera} & \texttt{mutex\_camera} e \texttt{camera\_event\_var}. \\
\bottomrule
\end{longtable}
...
O primeiro passo da implementação foi definir estruturas simples para representar as mensagens trocadas entre as tarefas. Essa escolha evita passar vários parâmetros soltos entre threads e deixa explícito qual informação circula em cada buffer. \texttt{SensorData} representa a leitura combinada de LIDAR e encoder, \texttt{EncoderData} separa o fluxo usado pela tarefa de distância, e \texttt{SurfacePoint} representa o ponto reconstruído do teto.
...
Separar \texttt{EncoderData} de \texttt{SensorData} foi importante para evitar que duas tarefas consumidoras retirassem dados da mesma fila. A reconstrução consome o fluxo de sensores, enquanto a distância percorrida consome o fluxo específico do encoder.
...
A coordenada \texttt{x} é lida de \texttt{SharedRobotState}, atualizado pela tarefa de distância percorrida. Dessa forma, o ponto reconstruído combina posição horizontal e distância vertical filtrada.
...
A tarefa \texttt{DistanciaPercorrida} implementa a regra do enunciado: o encoder troca de estado a cada metro percorrido. Sempre que o valor atual do encoder difere do anterior, a posição horizontal é incrementada.
...
O estado do robô foi centralizado em \texttt{SharedRobotState} para que a posição possa ser usada por outras tarefas, como a reconstrução da superfície.
...
O controle de navegação foi implementado inicialmente como controlador proporcional. Essa escolha é suficiente para demonstrar a interação entre comando, estado do robô e atuadores, sem introduzir a complexidade completa de um PID na Etapa 1.

\begin{lstlisting}[style=cppstyle]
const double setpointVelocidade =
    emInspecao ? std::min(comando.j_sp_velocidade, 1)
               : comando.j_sp_velocidade;

const double erro = setpointVelocidade - velocidadeAtual;
const int aceleracao =
    static_cast<int>(std::clamp(kp * erro, -100.0, 100.0));
...
Quando o robô entra em inspeção, o setpoint é limitado para reduzir a velocidade. Essa decisão acompanha o comportamento pedido no enunciado: ao detectar falha, o robô deve navegar mais devagar e acionar a câmera.
...
std::thread distancia(DistanciaPercorrida,
    std::ref(encoderBuffer), std::ref(robotState));

std::thread reconstrucao(ReconstrucaoSuperficie,
    std::ref(sensorBuffer), std::ref(surfaceBuffer),
...
Esse teste valida o \texttt{EncoderBuffer} e o uso de \texttt{SharedRobotState} para compartilhar a posição com outras tarefas.
